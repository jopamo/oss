...
void initializeProcessTable(int maxProcesses);
...
int main(int argc, char * argv[]) {
  ...
  maxProcesses = sysconf(_SC_NPROCESSORS_ONLN); // Get the number of CPU cores
  ...
  initializeProcessTable(maxProcesses);
  ...
  while (1) {
    ...
    if (currentTime - lastWorkerCheck < WORKER_LAUNCH_INTERVAL) {
      //printf("Error: Worker launch interval not met. Waiting for the next interval.\n");
    }
    else if (currentSimul >= simul) {
      // Check if the limit of simultaneous processes has been reached
      printf("Error: Maximum number of simultaneous processes reached.\n");
    }
    else if (totalLaunched >= proc) {
      // Check if the total number of processes to be launched has been reached
      printf("Error: Total number of processes to be launched has been reached.\n");
    }
    else {
      // All conditions met, proceed to launch new worker processes
      ...
      totalLaunched++;
      //printf("Worker process launched successfully.\n");
    }
...
void initializeProcessTable(int maxProcesses) {
  processTable = (PCB * ) calloc(maxProcesses, sizeof(PCB));
  if (!processTable) {
...
void checkAndLaunchWorkers(SimulatedClock * simClock, PCB * processTable, int proc, int simul, int timelimitForChildren, int intervalInMsToLaunchChildren, int * currentSimul, time_t * lastLaunchTime) {
  ...
  if (difftime(currentTime, * lastLaunchTime) * 1000 >= intervalInMsToLaunchChildren) {
    ...
    for (int i = 0; i < maxProcesses && * currentSimul < simul; i++) {
