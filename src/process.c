...
void signalHandler(int sig) {
    ...
    if (sig == SIGINT || sig == SIGTERM) {
        ...
        // Handling SIGCHLD to reap terminated child processes
        int status;
        while (waitpid(-1, &status, WNOHANG) > 0) {
            // Optionally log child process termination
        }
    }
}
...
void setupSignalHandlers(void) {
    ...
    int signals[] = {SIGINT, SIGTERM, SIGCHLD};  // Including SIGCHLD if needed
